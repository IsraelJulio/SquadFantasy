import type { AthletePosition, DraftTeam, Formation, GameAthlete, GameCoach, GamePlayer, Opponent, Strategy } from '../types'
...
const clonePlayer = (player: FutsalPlayerJson): FutsalPlayerJson => ({ ...player })
const cloneCoach = (coach: FutsalCoachJson): FutsalCoachJson => ({ ...coach })
const cloneTeam = (team: FutsalTeamJson): FutsalTeamJson => ({ ...team, playerIds: [...(team.playerIds ?? [])] })
...
function getCoachForTeam(team: FutsalTeamJson) {
  return database.coaches.find((item) => item.id === team.coachId)
...
export function getPlayersByTeamId(teamId: string): FutsalPlayerJson[] {
  return database.players.filter((player) => player.teamId === teamId).map(clonePlayer)
}
...
export function validateFutsalDatabase(databaseToValidate: FutsalDatabaseJson): string[] {
  const errors: string[] = []
  const teamIds = new Set(databaseToValidate.teams.map((team) => team.id))
  const playerIds = new Set(databaseToValidate.players.map((player) => player.id))
  const coachIds = new Set(databaseToValidate.coaches.map((coach) => coach.id))

  for (const id of duplicateIds(databaseToValidate.teams)) errors.push(`ID de time duplicado: ${id}`)
  for (const id of duplicateIds(databaseToValidate.players)) errors.push(`ID de atleta duplicado: ${id}`)
  for (const id of duplicateIds(databaseToValidate.coaches)) errors.push(`ID de tecnico duplicado: ${id}`)

  for (const player of databaseToValidate.players) {
    if (!teamIds.has(player.teamId)) errors.push(`Atleta ${player.id} aponta para time inexistente: ${player.teamId}`)
    if (player.role !== 'PLAYER') errors.push(`Atleta ${player.id} deve ter role PLAYER`)
    if (!validPlayerPositions.includes(player.position)) errors.push(`Atleta ${player.id} tem posicao invalida: ${player.position}`)
    if (player.stamina !== 100) errors.push(`Atleta ${player.id} deve iniciar com stamina 100`)
    if (player.overall !== player.overallOriginal) errors.push(`Atleta ${player.id} deve iniciar com overall igual ao original`)
  ...
  for (const coach of databaseToValidate.coaches) {
    if (!teamIds.has(coach.teamId)) errors.push(`Tecnico ${coach.id} aponta para time inexistente: ${coach.teamId}`)
    if (coach.role !== 'COACH') errors.push(`Tecnico ${coach.id} deve ter role COACH`)
    if (coach.position !== 'TECNICO') errors.push(`Tecnico ${coach.id} deve ter posicao TECNICO`)
    if (coach.overall !== coach.overallOriginal) errors.push(`Tecnico ${coach.id} deve iniciar com overall igual ao original`)
    if (coach.overall < minOverall || coach.overall > maxOverall) errors.push(`Tecnico ${coach.id} tem overall fora da faixa permitida`)
  ...
  for (const team of databaseToValidate.teams) {
    ...
    if (teamCoachCount > 1) errors.push(`Time ${team.id} deve ter no maximo 1 tecnico`)

    for (const id of duplicateIds(teamPlayerIds.map((playerId) => ({ id: playerId })))) {
      errors.push(`Time ${team.id} tem atleta duplicado em playerIds: ${id}`)
    }

    for (const playerId of teamPlayerIds) {
      if (coachIds.has(playerId)) errors.push(`Time ${team.id} colocou tecnico em playerIds: ${playerId}`)
      if (!playerIds.has(playerId)) errors.push(`Time ${team.id} aponta para atleta inexistente: ${playerId}`)
      const player = databaseToValidate.players.find((item) => item.id === playerId)
      if (player && player.teamId !== team.id) errors.push(`Atleta ${player.id} esta vinculado ao time ${player.teamId}, mas aparece em ${team.id}`)
      if (assignedPlayerIds.has(playerId)) errors.push(`Atleta ${playerId} aparece em mais de um time`)
