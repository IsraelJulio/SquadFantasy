...
import type { Difficulty, GamePlayer } from '../types'
...
export function PlayerCard({ player, difficulty, onSelect, selected = false, compact = false }: PlayerCardProps) {
  const coach = player.position === 'TECNICO'
  const showOverall = shouldShowPlayerOverall(difficulty)
  const rare = showOverall && player.overallOriginal >= 86
  const rating = showOverall ? player.overall : '?'
  ...
  return (
    <article className={`player-card ${coach ? 'player-card--coach' : ''} ${rare ? 'player-card--rare' : ''} ${selected ? 'player-card--selected' : ''} ${compact ? 'player-card--compact' : ''}`}>
      ...
      <div className="player-card__name"><h3>{player.name}</h3><p>{coach ? 'Técnico' : player.position} · overall base {showOverall ? player.overallOriginal : '?'}</p></div>
