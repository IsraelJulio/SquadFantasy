...
export function PlayerCard({ player, onSelect, selected = false, compact = false }: PlayerCardProps) {
  ...
  return (
    <article className={`player-card ${coach ? 'player-card--coach' : ''} ${rare ? 'player-card--rare' : ''} ${selected ? 'player-card--selected' : ''} ${compact ? 'player-card--compact' : ''}`}>
      ...
      <div className="player-card__top"><div className="overall"><strong>{player.overall}</strong><span>OVR</span></div><div className="player-card__identity"><span className="position-badge">{positionCode[player.position]}</span></div></div>
      <div className="player-silhouette" aria-hidden="true"><span>{player.name.split(' ').map((part) => part[0]).join('').slice(0, 2)}</span></div>
      <div className="player-card__name"><h3>{player.name}</h3><p>{coach ? 'Técnico' : player.position} · overall base {player.overallOriginal}</p></div>
      {!compact && <div className="overall-only"><span>FORÇA PRINCIPAL</span><strong>{player.overall}</strong><small>OVERALL</small></div>}
