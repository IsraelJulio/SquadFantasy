...
import {
  athletesFrom,
  calculateActiveLineupStrength,
  calculateComebackBoost,
  coachFrom,
  createDefaultLineup,
  updateTeamStamina,
  type SquadStrength,
...
import {
  ...
  normalizeFatigueFactor,
} from "./staminaRules";

export const OPPONENT_GOAL_CHANCE_CRITICAL_USER_STAMINA_MULTIPLIER = 3.0;
...
function event(
  ...
  values: Omit<
    MatchTimelineEvent,
    "id" | "minute" | "period" | "userScore" | "opponentScore"
...
function cloneOpponentAthlete(
  player: GameAthlete,
  opponentId: string,
): GameAthlete {
  return {
    ...player,
    id: `opponent-${opponentId}-${player.id}`,
    stamina: 100,
    overall: player.overallOriginal,
    fatigueFactor: normalizeFatigueFactor(player.fatigueFactor),
  };
}
...
function createOpponentCoach(opponent: Opponent): GameCoach {
  ...
  if (coach)
    return {
      ...coach,
      id: `opponent-coach-${opponent.id}-${coach.id}`,
      overall: coach.overallOriginal,
    };
...
function createOpponentTeam(opponent: Opponent) {
  const players = athletesFrom(opponent.players).map((player) =>
    cloneOpponentAthlete(player, opponent.id),
  );
  ...
  return {
    players,
    coach: createOpponentCoach(opponent),
    activeIds,
    benchIds: players
...
export function createInitialMatchState(
  ...
  const userPlayers = athletesFrom(plan.squad).map((player) => ({
    ...player,
    fatigueFactor: normalizeFatigueFactor(player.fatigueFactor),
    stamina: 100,
    overall: player.overallOriginal,
...
const playersByIds = (players: GameAthlete[], ids: string[]) =>
  ids
...
function chooseTopScorerOfMatch(state: LiveMatchState) {
  ...
  const topGoals = Math.max(
    0,
    ...[...scorers.values()].map((scorer) => scorer.goals),
  );
  if (topGoals === 0) return null;

  const leaders = [...scorers.values()].filter(
    (scorer) => scorer.goals === topGoals,
  );
  const random = seededRandom(
    hashSeed(
      `${state.plan.campaignId}-${state.plan.matchNumber}-${state.userScore}-${state.opponentScore}-motm-${leaders.map((leader) => leader.name).join("-")}`,
    ),
  );
...
export function calculateLiveMatchStrengths(
  state: LiveMatchState,
  opponent: Opponent,
): LiveMatchStrengths {
  const active = playersByIds(state.userPlayers, state.activeIds);
  const opponentActive = playersByIds(
    state.opponentPlayers,
    state.opponentActiveIds,
  );
  const tacticalMatchup = calculateTacticalMatchup(
    state.plan.strategy,
    opponent.strategy,
  );
  const comebackBoost = calculateComebackBoost(state.plan.losingStreak);
  const user = calculateActiveLineupStrength(
    active,
    coachFrom(state.plan.squad),
    state.plan.formation,
    state.plan.strategy,
    comebackBoost,
    tacticalMatchup.userBoost,
    1,
  );
  const opponentStrength = calculateActiveLineupStrength(
    opponentActive,
    state.opponentCoach,
    "DIAMOND_3_1",
    opponent.strategy,
    1,
    tacticalMatchup.opponentBoost,
    1,
  );
  const experienceBoost = calculateDifficultyBoost(
    state.plan.difficulty,
    user.finalStrength / comebackBoost,
    opponentStrength.finalStrength,
  );
  return {
    user: {
      ...user,
      finalStrength: user.finalStrength * experienceBoost,
      experienceBoost,
    },
...
function userScorer(players: GameAthlete[], random: () => number) {
  const candidates = [...players]
    ...
    .slice(0, 4);
  return (
    candidates[Math.floor(random() * Math.max(1, candidates.length))]?.name ??
    "Seu ala"
  );
}

function clampGoalChance(chance: number) {
  return Math.max(0.02, Math.min(0.14, chance));
}

export function calculateMinuteGoalChances(
  userStrength: number,
  opponentStrength: number,
  userActivePlayers: Array<Pick<GameAthlete, "stamina">>,
) {
  const strengthEdge = userStrength - opponentStrength;
  const userGoalChance = clampGoalChance(0.065 + strengthEdge / 650);
  const opponentBaseGoalChance = clampGoalChance(0.065 - strengthEdge / 650);
  const hasCriticalUserStamina = userActivePlayers.some(
    (player) => player.stamina < CRITICAL_STAMINA_THRESHOLD,
  );
  const opponentGoalChance = hasCriticalUserStamina
    ? opponentBaseGoalChance *
      OPPONENT_GOAL_CHANCE_CRITICAL_USER_STAMINA_MULTIPLIER
    : opponentBaseGoalChance;

  return { userGoalChance, opponentGoalChance };
}

function applyOpponentSubstitution(
  state: LiveMatchState,
  random: () => number,
): LiveMatchState {
  ...
  return {
    ...state,
    opponentActiveIds: state.opponentActiveIds.map((id) =>
      id === outgoing.id ? incoming.id : id,
    ),
    opponentBenchIds: state.opponentBenchIds.map((id) =>
      id === incoming.id ? outgoing.id : id,
    ),
    events: [...state.events, substitutionEvent],
  };
}

export function simulateNextMinute(
  state: LiveMatchState,
  opponent: Opponent,
): LiveMatchState {
  const minute = state.minute + 1;
  const userPlayers = updateTeamStamina(
    state.userPlayers,
    state.activeIds,
    state.plan.strategy,
    minute,
  );
  const opponentPlayers = updateTeamStamina(
    state.opponentPlayers,
    state.opponentActiveIds,
    opponent.strategy,
    minute,
  ...
  const random = seededRandom(
    hashSeed(
      `${state.plan.campaignId}-${state.plan.matchNumber}-${minute}-${lineupSignature}-${state.userScore}-${state.opponentScore}`,
    ),
  );
  ...
  const { userGoalChance, opponentGoalChance } = calculateMinuteGoalChances(
    userStrength,
    opponentStrength,
    active,
  );
  const userEventShare = Math.max(
    0.35,
    Math.min(0.65, userStrength / Math.max(1, userStrength + opponentStrength)),
  );
  const roll = random();
  ...
  if (roll < userGoalChance) {
    ...
    events.push(
      event(
        { ...next, userScore, opponentScore },
        {
          type: "goal",
          team: "user",
          playerName,
          description: `GOL! ${playerName} gira rápido e finaliza no canto.`,
          userScore,
          opponentScore,
        },
      ),
    ...
    events.push(
      event(
        { ...next, userScore, opponentScore },
        {
          ...
          team: "opponent",
          playerName,
          description: `Gol do adversário. ${playerName} conclui a troca de passes.`,
          userScore,
          opponentScore,
        },
      ),
    ...
    const descriptions = {
      chance:
        team === "user"
          ? "Seu time acelera e cria uma chance perigosa."
          : `${opponent.name} chega com perigo pela ala.`,
      save:
        team === "user"
          ? "Seu goleiro fecha o ângulo e faz grande defesa."
          : `O goleiro do ${opponent.name} evita o gol.`,
      foul:
        team === "user"
          ? "Falta sofrida pelo seu time perto da área."
          : "Falta perigosa para o adversário.",
    };
    events.push(event(next, { type, team, description: descriptions[type] }));
  }

  const tiredUser = active
    .filter((player) => player.stamina < 35)
    .sort((a, b) => b.overallOriginal - a.overallOriginal)[0];
  const tiredOpponentCount = opponentActive.filter(
    (player) => player.stamina < 35,
  ).length;
  const userTiredCount = active.filter((player) => player.stamina < 35).length;
  const shouldNarrateFatigue =
    !events.some((item) => item.minute === minute && item.type === "fatigue") &&
    ((tiredUser && random() < 0.06) ||
      ((userTiredCount >= 3 || tiredOpponentCount >= 3) && random() < 0.1));
  ...
  return applyOpponentSubstitution(
    { ...next, userScore, opponentScore, events },
    random,
  );
}

export function createSubstitutionTimelineEvent(
  state: LiveMatchState,
  outgoing: GameAthlete,
  incoming: GameAthlete,
): MatchTimelineEvent {
  return event(state, {
    type: "substitution",
    team: "user",
...
export function applySubstitution(
  ...
  const outgoing = state.userPlayers.find(
    (player) => player.id === substitution.playerOutId,
  );
  const incoming = state.userPlayers.find(
    (player) => player.id === substitution.playerInId,
  );
  if (!outgoing || !incoming || outgoing.position !== incoming.position)
    return state;
  if (
    !state.activeIds.includes(outgoing.id) ||
    !state.benchIds.includes(incoming.id)
  )
    return state;
  return {
    ...state,
    activeIds: state.activeIds.map((id) =>
      id === outgoing.id ? incoming.id : id,
    ),
    benchIds: state.benchIds.map((id) =>
      id === incoming.id ? outgoing.id : id,
    ),
    events: [
      ...state.events,
      createSubstitutionTimelineEvent(state, outgoing, incoming),
    ],
  };
}
...
export function addSystemEvent(
  state: LiveMatchState,
  type: "kick-off" | "half-time" | "second-half" | "full-time",
  description: string,
): LiveMatchState {
  return {
    ...state,
    events: [
      ...state.events,
      event(state, { type, team: "neutral", description }),
    ],
  };
}
...
export function finalizeMatch(
  ...
  const tiedKnockout =
    state.plan.stage !== "Fase de grupos" &&
    state.userScore === state.opponentScore;
  if (tiedKnockout && shootout?.status !== "finished")
    throw new Error(
      "Uma partida eliminatória empatada só pode terminar após os pênaltis.",
    );
  const active = playersByIds(state.userPlayers, state.activeIds);
  const result = tiedKnockout
    ? shootout!.winner === "user"
      ? "victory"
      : "defeat"
    : state.userScore > state.opponentScore
      ? "victory"
      : state.userScore < state.opponentScore
        ...
        : "draw";
  const bestByOverall =
    [...active].sort((a, b) => b.overall - a.overall)[0]?.name ??
    "Destaque do futsal";
  ...
  const match: GameMatch = {
    ...
    result,
    summary: tiedKnockout
      ? `Após o empate, a vaga foi decidida nos pênaltis: ${result === "victory" ? "classificação do seu time" : `classificação do ${opponent.name}`}.`
      : result === "victory"
        ? "Seu quinteto controlou os momentos decisivos e confirmou a vitória."
        : result === "defeat"
...
export function simulateMatch(
  ...
  for (let minute = 0; minute < 40; minute += 1)
    state = simulateNextMinute(state, opponent);
  if (stage !== "Fase de grupos" && state.userScore === state.opponentScore) {
    let shootout = startPenaltyShootout(createPenaltyShootout());
    const random = seededRandom(
      hashSeed(`${campaignId}-${matchNumber}-penalties`),
    );
    while (shootout.status !== "finished")
      shootout = takeNextPenalty(state, shootout, random);
